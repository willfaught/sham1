(module haskell-typechecker mzscheme
    ...
    (match term
      ...
      (($ declaration-term p e t) (if (equal? (length p) 1)
                                      (let ((e-type (term-type context e)))
                                        (if (equal? t e-type) t (error 'term-type "expected type '~a', found type '~a'" t e-type)))
                                      (let ((f-type (term-type context (make-function-term (cdr p) e t))))
                                        (if (equal? t f-type) t (error 'term-type "expected type '~a', found type '~a'" t f-type)))))
      (($ float-term _) (make-float-type))
      (($ function-term p b t) (let ((f-type (function-type-types t)))
                                 (if (equal? (+ (length p) 1) (length f-type))
                                   (let ((b-type (term-type (append (zip p (reverse (cdr (reverse f-type)))) context) b)))
                                     (if (equal? b-type (car (reverse f-type))) t (error 'term-type "body type doesn't match given type")))
  ...
  (define (convert-type type)
    (match type
      ...
      ("Char" (make-character-type))
      ("Int" (make-integer-type))
      ("Integer" (make-integer-type))
      ("Float" (make-float-type))))
