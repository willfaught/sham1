...
(module compiler mzscheme
  ...
  (define (haskell-contract type)
    (match type
      ...
      (($ list-type type) `(cons-immutable/c promise? promise?))
      (($ tuple-type types) `(vector-immutable/c ,(map (lambda (x) `(flat-contract promise?)) types)))
