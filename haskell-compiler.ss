(module haskell-compiler mzscheme
  ...
  (define-struct (declaration-term term) (patterns expression types) #f)
  (define-struct (float-term term) (float) #f)
  (define-struct (function-term term) (patterns body types) #f)
  ...
  (define compile-expression
    (match-lambda (($ application-term f a) (compile-eapp f (reverse a)))
                  ...
                  (($ function-term p b t) (if (null? p) (compile-expression b) `(match-lambda (,(string->symbol (car p)) ,(compile-expression (make-function-term (cdr p) b (cdr t)))))))
  ...
  #;(define compilation-tests
      (list (make-test "eapp 1" (make-application-term (make-identifier-term "x") (list (make-integer-term "4"))) '((force x) (delay 4)))
            ...
            (make-test "edecl 1" (make-declaration-term (list "x") (make-integer-term "2")) '(define x (delay 2)))
            (make-test "edecl 2" (make-declaration-term (list "x" "y") (make-integer-term "2")) '(define x (delay (match-lambda (y 2)))))
            (make-test "edecl 3" (make-declaration-term (list "x" "_" "y") (make-integer-term "2")) '(define x (delay (match-lambda (_ (match-lambda (y 2)))))))
            (make-test "efun 1" (make-function-term (list "x") (list (make-integer-type)) (make-identifier-term "x")) '(match-lambda (x (force x))))
            (make-test "efun 2" (make-function-term (list "x" "y") (list (make-integer-type)) (make-identifier-term "x")) '(match-lambda (x (match-lambda (y (force x))))))
            ...
            (make-test "elet 1" (make-let-term (list (make-declaration-term (list "x") (make-integer-term "2"))) (make-integer-term "3")) '(begin (define x (delay 2)) 3))
            (make-test "elet 2" (make-let-term (list (make-declaration-term (list "x") (make-integer-term "2")) (make-declaration-term (list "y") (make-integer-term "3"))) (make-integer-term "4")) '(begin (define x (delay 2)) (define y (delay 3)) 4))
            ...
            (make-test "etup 1" (make-tuple-term (list (make-integer-term "2") (make-character-term "c"))) '(((lambda (x1) (lambda (x2) (vector x1 x2))) (delay 2)) (delay #\c)))
