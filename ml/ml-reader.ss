...
(module ml-reader mzscheme
  ...
  (define (convert-type type)
    (match type
      (($ arrow-type _ p r) `(haskell.make-function-type ,(convert-type p) ,(convert-type p)))
      (($ bool-type _) `(haskell.make-type-constructor "Bool"))
      (($ int-type _) `(haskell.make-integer-type))
      (($ forall-type _ _ t) (convert-type t))
      ((? lump-type? _) (error 'convert-type "I didn't think the lump type was even used"))
      (($ str-type _) `(haskell.make-list-type (haskell.make-character-type)))
      (($ tuple-type _ t) `(haskell.make-tuple-type (list ,@(map convert-type t))))
      (($ tyvar _ i) `(haskell.make-type-variable ,(list->string (filter (lambda (x) (not (equal? x #\_))) (string->list (symbol->string i))))))
      (($ unit-type _) `(haskell.make-type-constructor "()"))
      ((? user-type? _) (error 'convert-type "user type not supported by ML yet"))))
  
  ; type-exports :: (string) (string) -> syntax-object
  (define (type-exports identifiers types)
    #`(define ml:types `#,(map (lambda (i t) #`(#,i ,#,t)) identifiers (map convert-type types))))
