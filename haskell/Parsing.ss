(module Parsing mzscheme
  ...
  (provide/contract (declarationParser (-> string? (-> string? HaskellSyntax?)))
                    (expressionParser (-> string? (-> string? HaskellSyntax?)))
                    (importParser (-> string? (-> string? HaskellSyntax?)))
                    (languageLexer (-> string? (-> string? HaskellSyntax?)))
    ...
    (parser (src-pos)
            ...
            (error (lambda (token-ok token-name token-value start-pos end-pos)
                     (raise-read-error (format "error: malformed ~a (~a ~a:~a): ~a"
                                       ...
                                       (position-offset start-pos)
                                       (- (position-offset end-pos) (position-offset start-pos)))))
            (grammar (nt-module ((t-module nt-modid t-where nt-body) (make-Module $2 null (list-ref $4 0) (list-ref $4 1)))
                     ...
                     (nt-impdecl ((t-import nt-impdecl-2 t-string t-as nt-modid nt-impdecl-3)
                                  (make-Import $2 (charsToString $3) $5 $6)))
                     (nt-impdecl-2 (() #f)
                                   ((t-varid) (if (not (equal? $1 "qualified")) (varidError "qualified" $1) #t)))
                     (nt-impdecl-3 (() (make-Nothing))
                                   ((nt-impspec) $1))
                     (nt-impspec ((t-lrbracket nt-impspec-2 t-rrbracket) (if (null? $2) (make-Nothing) (make-Just (list #f $2))))
                                 ((t-hiding t-lrbracket nt-impspec-2 t-rrbracket) (if (null? $3) (make-Nothing) (make-Just (list #t $3)))))
                     (nt-impspec-2 (() null)
                                   ((nt-import) (list $1))
                                   ((nt-impspec-2 t-comma) $1)
                                   ((nt-impspec-2 t-comma nt-import) (cons $3 $1)))
                     (nt-import ((nt-cname) (list $1 $1))
                                ((t-string t-as nt-cname) (list (charsToString $1) $3)))
                     (nt-cname ((nt-var) $1)
                               ((nt-con) $1))
                     (nt-topdecls (() null)
  ...
  (define/contract makeParser (-> string? integer? (-> string? HaskellSyntax?))
    (lambda (sourceName parserIndex)
  ...
  (define/contract declarationParser (-> string? (-> string? HaskellSyntax?))
    (lambda (name)
      (makeParser name 3)))
  
  (define/contract expressionParser (-> string? (-> string? HaskellSyntax?))
    (lambda (name)
      (makeParser name 0)))
  
  (define/contract importParser (-> string? (-> string? HaskellSyntax?))
    (lambda (name)
      (makeParser name 1)))
  
  (define/contract moduleParser (-> string? (-> string? HaskellSyntax?))
    (lambda (name)
      (makeParser name 2)))
  
  (define/contract typeParser (-> string? (-> string? HaskellSyntax?))
    (lambda (name)
      (makeParser name 4))))