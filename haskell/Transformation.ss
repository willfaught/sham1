  ...
  (define preludeTypes
    (let* ((typeParsers (parsers "Transformation"))
      ...
      (append (dataTypes (transformSyntax (parseD "data Bool = True | False")))
              (list (list "error" (parseT "[Char] -> a"))
                    (list "floatAdd" (parseT "Float -> Float -> Float"))
                    (list "floatDivide" (parseT "Float -> Float -> Float"))
                    (list "floatMultiply" (parseT "Float -> Float -> Float"))
                    (list "floatSubtract" (parseT "Float -> Float -> Float"))
                    (list "fst" (parseT "(a, b) -> a"))
                    (list "head" (parseT "[a] -> a"))
                    (list "intAdd" (parseT "Int -> Int -> Int"))
                    (list "intDivide" (parseT "Int -> Int -> Int"))
                    (list "intMultiply" (parseT "Int -> Int -> Int"))
                    (list "intSubtract" (parseT "Int -> Int -> Int"))
